...
class AboutMaps extends KoanSuite {
  ...
  // Pretty cool. You could be "lazy" when building these programatically
  ...
  // Cooooool.  There must be serious compiler magic going on
  ...
  koan("Map elements can be removed easily") {
    val myMap = Map("MI" -> "Michigan", "OH" -> "Ohio", "WI" -> "Wisconsin", "IA" -> "Iowa")
    // Oh cool. Are wonder which other operators are added as unique methods...
