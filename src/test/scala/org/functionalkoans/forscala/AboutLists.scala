...
class AboutLists extends KoanSuite {
  ...
  koan("Nil lists are identical, even of different types") {
    ...
    // TODO: does this mean that Nil is the same referenced object
    // everywhere?
  ...
  koan("Functions over lists can use _ as shorthand") {
    ...
    // Weird-lookin but I really like the brevity
    a.map {_ * 2} should equal(List(2, 4, 6))

    a.filter {_ % 2 == 0} should equal(List(2))
  }

  // As mentioned in class, {} and () are mainly differentiated
  // by {}'s ability to declare vals and such
  ...
  // More strict but similar to Ruby's reduce.  I'm a fan
